...
/* START: fig5_15.txt */
        HashTable
        Hash_initializeTable( int TableSize )
        {
            HashTable H;
            int i;
...
/* START: fig5_16.txt */
        Position
        Hash_find( ElementType Key, HashTable H )
        {
            Position CurrentPos;
            int CollisionNum;
...
/* START: fig5_17.txt */
        void
        Hash_insert( ElementType Key, HashTable H )
        {
            Position Pos;
...
/* 3*/      H = Hash_initializeTable( 2 * OldSize );

            /* Scan through old table, reinserting into new */
/* 4*/      for( i = 0; i < OldSize; i++ )
/* 5*/          if( OldCells[ i ].Info == Legitimate )
/* 6*/              Hash_insert( OldCells[ i ].Element, H );
...
/* END */
        ...
        ElementType
        Hash_retrieve( Position P, HashTable H )
        {
            return H->TheCells[ P ].Element;
        }

        void
        Hash_destroyTable( HashTable H )
        {
            free( H->TheCells );
            free( H );
        }
