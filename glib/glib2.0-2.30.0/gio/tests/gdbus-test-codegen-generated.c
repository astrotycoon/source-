  ...
  GParamSpec   *pspec)
{
  ...
  if (!_g_value_equal (value, &skeleton->priv->properties->values[prop_id - 1]))
    {
      if (g_dbus_interface_skeleton_get_connection (G_DBUS_INTERFACE_SKELETON (skeleton)) != NULL)
        _foo_igen_inline_docs_schedule_emit_changed (skeleton, _foo_igen_inline_docs_property_info_pointers[prop_id - 1], prop_id, &skeleton->priv->properties->values[prop_id - 1]);
      g_value_copy (value, &skeleton->priv->properties->values[prop_id - 1]);
      g_object_notify_by_pspec (object, pspec);
    }
  g_mutex_unlock (skeleton->priv->lock);
...
foo_igen_inline_docs_skeleton_get_property2 (FooiGenInlineDocs *object)
{
  ...
  const gchar *value;
  g_mutex_lock (skeleton->priv->lock);
  value = g_value_get_string (&(skeleton->priv->properties->values[1]));
  g_mutex_unlock (skeleton->priv->lock);
  return value;
}
...
foo_igen_inline_docs_skeleton_get_property4 (FooiGenInlineDocs *object)
{
  ...
  const gchar *value;
  g_mutex_lock (skeleton->priv->lock);
  value = g_value_get_string (&(skeleton->priv->properties->values[3]));
  g_mutex_unlock (skeleton->priv->lock);
  return value;
}
...
foo_igen_inline_docs_skeleton_get_fancy_property (FooiGenInlineDocs *object)
{
  ...
  const gchar *value;
  g_mutex_lock (skeleton->priv->lock);
  value = g_value_get_string (&(skeleton->priv->properties->values[5]));
  g_mutex_unlock (skeleton->priv->lock);
  return value;
}
  ...
  GParamSpec   *pspec)
{
  ...
  if (!_g_value_equal (value, &skeleton->priv->properties->values[prop_id - 1]))
    {
      if (g_dbus_interface_skeleton_get_connection (G_DBUS_INTERFACE_SKELETON (skeleton)) != NULL)
        _foo_igen_test_ugly_case_interface_schedule_emit_changed (skeleton, _foo_igen_test_ugly_case_interface_property_info_pointers[prop_id - 1], prop_id, &skeleton->priv->properties->values[prop_id - 1]);
      g_value_copy (value, &skeleton->priv->properties->values[prop_id - 1]);
      g_object_notify_by_pspec (object, pspec);
    }
  g_mutex_unlock (skeleton->priv->lock);
...
foo_igen_test_ugly_case_interface_skeleton_get_ugly_name (FooiGenTesTuglyCASEInterface *object)
{
  ...
  gint value;
  g_mutex_lock (skeleton->priv->lock);
  value = g_value_get_int (&(skeleton->priv->properties->values[0]));
  g_mutex_unlock (skeleton->priv->lock);
  return value;
}
  ...
  GParamSpec   *pspec)
{
  ...
  if (!_g_value_equal (value, &skeleton->priv->properties->values[prop_id - 1]))
    {
      if (g_dbus_interface_skeleton_get_connection (G_DBUS_INTERFACE_SKELETON (skeleton)) != NULL)
        _foo_igen_oldie_interface_schedule_emit_changed (skeleton, _foo_igen_oldie_interface_property_info_pointers[prop_id - 1], prop_id, &skeleton->priv->properties->values[prop_id - 1]);
      g_value_copy (value, &skeleton->priv->properties->values[prop_id - 1]);
      g_object_notify_by_pspec (object, pspec);
    }
  g_mutex_unlock (skeleton->priv->lock);
  ...
  GParamSpec   *pspec)
{
  ...
  if (!_g_value_equal (value, &skeleton->priv->properties->values[prop_id - 1]))
    {
      if (g_dbus_interface_skeleton_get_connection (G_DBUS_INTERFACE_SKELETON (skeleton)) != NULL)
        _foo_igen_unknown_xml_tags_schedule_emit_changed (skeleton, _foo_igen_unknown_xml_tags_property_info_pointers[prop_id - 1], prop_id, &skeleton->priv->properties->values[prop_id - 1]);
      g_value_copy (value, &skeleton->priv->properties->values[prop_id - 1]);
      g_object_notify_by_pspec (object, pspec);
    }
  g_mutex_unlock (skeleton->priv->lock);
 ...
 * @object: A #FooiGenObject.
 *
 * Like foo_igen_object_get_bar() but doesn' increase the reference count on the returned object.
 *
 ...
 * @object: A #FooiGenObject.
 *
 * Like foo_igen_object_get_bar_frobnicator() but doesn' increase the reference count on the returned object.
 *
 * <warning>It is not safe to use the returned object if you are on another thread than the one where the #GDBusObjectManagerClient or #GDBusObjectManagerServer for @object is running.</warning>
 *
 ...
 * @object: A #FooiGenObject.
 *
 * Like foo_igen_object_get_baz() but doesn' increase the reference count on the returned object.
 *
 * <warning>It is not safe to use the returned object if you are on another thread than the one where the #GDBusObjectManagerClient or #GDBusObjectManagerServer for @object is running.</warning>
 *
 ...
 * @object: A #FooiGenObject.
 *
 * Like foo_igen_object_get_com_acme_coyote() but doesn' increase the reference count on the returned object.
 *
 * <warning>It is not safe to use the returned object if you are on another thread than the one where the #GDBusObjectManagerClient or #GDBusObjectManagerServer for @object is running.</warning>
 *
 ...
 * @object: A #FooiGenObject.
 *
 * Like foo_igen_object_get_rocket123() but doesn' increase the reference count on the returned object.
 *
 * <warning>It is not safe to use the returned object if you are on another thread than the one where the #GDBusObjectManagerClient or #GDBusObjectManagerServer for @object is running.</warning>
 *
 ...
 * @object: A #FooiGenObject.
 *
 * Like foo_igen_object_get_bat() but doesn' increase the reference count on the returned object.
 *
 * <warning>It is not safe to use the returned object if you are on another thread than the one where the #GDBusObjectManagerClient or #GDBusObjectManagerServer for @object is running.</warning>
 *
 ...
 * @object: A #FooiGenObject.
 *
 * Like foo_igen_object_get_authorize() but doesn' increase the reference count on the returned object.
 *
 * <warning>It is not safe to use the returned object if you are on another thread than the one where the #GDBusObjectManagerClient or #GDBusObjectManagerServer for @object is running.</warning>
 *
 ...
 * @object: A #FooiGenObject.
 *
 * Like foo_igen_object_get_method_threads() but doesn' increase the reference count on the returned object.
 *
 * <warning>It is not safe to use the returned object if you are on another thread than the one where the #GDBusObjectManagerClient or #GDBusObjectManagerServer for @object is running.</warning>
 *
 ...
 * @object: A #FooiGenObject.
 *
 * Like foo_igen_object_get_inline_docs() but doesn' increase the reference count on the returned object.
 *
 * <warning>It is not safe to use the returned object if you are on another thread than the one where the #GDBusObjectManagerClient or #GDBusObjectManagerServer for @object is running.</warning>
 *
 ...
 * @object: A #FooiGenObject.
 *
 * Like foo_igen_object_get_changing_interface_v1() but doesn' increase the reference count on the returned object.
 *
 * <warning>It is not safe to use the returned object if you are on another thread than the one where the #GDBusObjectManagerClient or #GDBusObjectManagerServer for @object is running.</warning>
 *
 ...
 * @object: A #FooiGenObject.
 *
 * Like foo_igen_object_get_changing_interface_v2() but doesn' increase the reference count on the returned object.
 *
 * <warning>It is not safe to use the returned object if you are on another thread than the one where the #GDBusObjectManagerClient or #GDBusObjectManagerServer for @object is running.</warning>
 *
 ...
 * @object: A #FooiGenObject.
 *
 * Like foo_igen_object_get_changing_interface_v10() but doesn' increase the reference count on the returned object.
 *
 * <warning>It is not safe to use the returned object if you are on another thread than the one where the #GDBusObjectManagerClient or #GDBusObjectManagerServer for @object is running.</warning>
 *
 ...
 * @object: A #FooiGenObject.
 *
 * Like foo_igen_object_get_test_ugly_case_interface() but doesn' increase the reference count on the returned object.
 *
 * <warning>It is not safe to use the returned object if you are on another thread than the one where the #GDBusObjectManagerClient or #GDBusObjectManagerServer for @object is running.</warning>
 *
 ...
 * @object: A #FooiGenObject.
 *
 * Like foo_igen_object_get_oldie_interface() but doesn' increase the reference count on the returned object.
 *
 * <warning>It is not safe to use the returned object if you are on another thread than the one where the #GDBusObjectManagerClient or #GDBusObjectManagerServer for @object is running.</warning>
 *
 ...
 * @object: A #FooiGenObject.
 *
 * Like foo_igen_object_get_unknown_xml_tags() but doesn' increase the reference count on the returned object.
 *
 * <warning>It is not safe to use the returned object if you are on another thread than the one where the #GDBusObjectManagerClient or #GDBusObjectManagerServer for @object is running.</warning>
 *
 ...
 * @object: A #FooiGenObject.
 *
 * Like foo_igen_object_get_fdpassing() but doesn' increase the reference count on the returned object.
 *
 * <warning>It is not safe to use the returned object if you are on another thread than the one where the #GDBusObjectManagerClient or #GDBusObjectManagerServer for @object is running.</warning>
 *
